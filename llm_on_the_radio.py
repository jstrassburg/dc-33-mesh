...
You're brief (≤200 chars), sarcastic, and not afraid to troll.
...
def on_receive(packet, interface):
    try:
        if "decoded" in packet and packet["decoded"]["portnum"] == "TEXT_MESSAGE_APP":
            ...
            print(
                f"Incomming message: [Channel: {packet_channel}, From: {message_from}] {message_string}"
            )

            # Filter by the specified channel (or show all if channel_index is -1)
            if channel_index == -1 or packet_channel == channel_index:
                if message_string.startswith(WELCOME_MESSAGE_TRIGGER):
                    ...
                    print(
                        f"Message ignored as it doesn't start with {WELCOME_MESSAGE_TRIGGER}."
                    )
            else:
                print(
                    f"Message on channel {packet_channel} ignored as it wasn't channel {channel_index}."
                )
...
def main():
    ...
    try:
        if channel_index != -1:
            welcome_message = f"Hello DEF CON 33! I am TemuGPT - your unhelpful AI chat bot on the radio. Start a message with {WELCOME_MESSAGE_TRIGGER} to get my attention."
            interface.sendText(welcome_message, channelIndex=channel_index)
        while True:
            if channel_index == -1:
                print(
                    "Listening for messages on all channels. Choose one channel to be able to send..."
                )
            else:
